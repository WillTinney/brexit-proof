...
feature 'Child Create Page', js: false do
  ...
  it 'has a type field' do
    # page.find_by_id( 'my_element_id' ) ; elem.value.should eq Date.today.to_s
    expect(page).to have_field('assignee_type')
  end

  it 'has a relationship field' do
    expect(page).to have_field('assignee_relationship', visible: false)
  end

  it 'type is recipient' do
    expect('assignee_type').to have_value('Recipient')
  end

  it 'relationship is child' do
    expect('assignee_relationship').to have_value('Child')
  end
